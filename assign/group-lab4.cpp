...
class Trinomial : public Complex {
    friend ostream& operator<<(ostream&, const Trinomial&);
protected:
    double c;
public:
    // derived class constructor that uses base class constructor
    Trinomial(double x, double y, double z): Complex(x, y), c{z} {}
    // overloaded operation
    Trinomial operator+(Trinomial& num) const {
        double c1 = a + num.a;
        double c2 = b + num.b;
        double c3 = c + num.c;
        return Trinomial(c1, c2, c3);
    }
};

ostream& operator<<(ostream& output, const Trinomial& obj) {
    if (obj.a != 0)
        output << obj.a << "x^2";
    if (obj.b != 0)
